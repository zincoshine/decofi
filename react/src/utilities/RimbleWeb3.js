...
class RimbleTransaction extends React.Component {
  ...
  initWeb3 = async () => {
    ...
    this.setState({ web3, ethereum: window.ethereum }, () => {
        ...
        await this.state.web3.eth
          ...
          .then(async accountBalance => {
            if (!isNaN(accountBalance)) {
              ...
              const daiBalance = await getDaiBalanceAction();

              // Only update if changed
              if (accountBalance !== this.state.accountBalance || daiBalance !== this.state.daiBalance) {
                this.setState({ accountBalance, daiBalance });
  ...
  determineAccountLowBalance = () => {
    ...
    this.setState({
      ...
      this.props.updateWeb3(this.state);
    });
      ...
      (error, signature) => {
        if (error) {
          ...
          window.toastProvider.addMessage(successMessage, {
            variant: "success",
            secondaryMessage: "Welcome to the Decentralized Community Financing dApp 🎉"
          });
  ...
  checkNetwork = async () => {
    ...
    this.setState({ network }, () => {
      if(network.isCorrectNetwork && this.state.account){
        this.connectAndValidateAccount()
      }
    });
  };
  ...
  pollAccountUpdates = () => {
    ...
    let accountInterval = setInterval(() => {
      ...
      if (window.ethereum.isConnected()) {
        const updatedAccount = window.ethereum.selectedAddress;
  ...
  componentDidMount() {
    ...
    this.setState({...this.props.web3Reducer}, ()=>{
      this.initWeb3();
    });
  }
...
const mapStateToProps = state => ({
  ...
  transactions: state.decofiUserReducer.monthlyStakeTransactions
});

export default connect(mapStateToProps,actionToProps)(RimbleTransaction);
