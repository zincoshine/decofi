...
import {
    updateMonthlyStakeTransactions,
    updateContractSubs,
} from 'reducers/decofiUserReducer';
import {
    createNewContractAction,
    addContractMembers,
    addContractWinner,
    updateDeployedContracts,
} from 'reducers/decofiAdminReducer';
                ...
                const finalList = contractDeployments
                    ? contractDeployments
                          .filter(txs => {
                              return (
                                  txs.creates &&
                                  txs.input.startsWith(
                                      SocialFundABI['bytecode'],
                                  )
                              );
                          })
                          .map(fintx => {
...
export const getAllTransactionsFromContract = async (
    contractAddress,
    targetAddress,
) => {
    const web3 = web3Factory();
    return web3.eth
        .getPastLogs({
            ...
            topics: [
                ...
                web3.currentProvider.selectedAddress.replace(
                    '0x',
        ...
        .then(async results => {
            const promises = results.map(async result => {
                ...
                return web3.eth
                    ...
                    .then(tx => {
                        const confirmations =
                            blockNumber > 0 ? blockNumber - tx.blockNumber : 0;
...
export const deploySocialFundContract = (contractParams, navigator) => {
    return (dispatch, getState) => {
        const web3 = web3Factory();
        const socFundContract = new web3.eth.Contract(
            SocialFundABI['abi'],
            '0xb7c7e45Ef6a8D46135BB519CDA1fFBeEF4845122',
        );
        ...
        socFundContract
            ...
            .then(newContract => {
                ...
                navigator.push('/admin/addmembers', {
                    ...contractParams,
                    contractAddress: newContract.options.address,
                });
            })
...
export const addMembersAction = (contractAddress, addressList, navigator) => {
    return (dispatch, getState) => {
        ...
        if (addressList.length === 1) {
            socFundContract.methods
                .addMember(addressList[0])
                .send({
                    from: web3.currentProvider.selectedAddress,
                })
                .then(receipt => {
                    dispatch(
                        addContractMembers(contractAddress, [...addressList]),
                    );
                    navigator.push('/admin', {
...
export const getMembersAction = async contractAddress => {
    ...
    return socFundContract.methods
        .listMembers()
        .call({
            from: web3.currentProvider.selectedAddress,
        })
...
export const syncDaiTransactionsAction = contractAddress => {
    return async (dispatch, getState) => {
        const {
            decofiUserReducer: { monthlyStakeTransactions = [] },
        } = getState();
        const transactions = await getAllTransactionsFromContract(
            '0xFf795577d9AC8bD7D90Ee22b6C1703490b6512FD',
            contractAddress,
        );
        ...
        allDaiTransactions.forEach(adt => {
            ...
            const mst = monthlyStakeTransactions.find(
                tx => tx.hash === adt.hash,
            );
            if (mst) {
                if (
                    mst.status !== status ||
                    mst.confirmations !== adt.confirmations
                ) {
                    results.push({
                        ...mst,
                        ...adt,
